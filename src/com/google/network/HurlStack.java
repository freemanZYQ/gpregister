...
public class HurlStack implements HttpStack {
    ...
    public  boolean isSocksProxy = false;
    ...
    private static void addBodyIfExists(HttpURLConnection connection, Request<?> request)
        ...
        if (body != null) {
            ...
            if(request.isRequestGzip()){
                System.out.println("gzipgzipgzip");
...
//            connection.addRequestProperty(HEADER_CONTENT_TYPE, request.getBodyContentType());
    ...
    protected HttpURLConnection createConnection(URL url) throws IOException {
        if(useProxy) {
//            GpRanking.logger.info("proxyHost:" + host + " port:" + port);
        }
        if(useProxy&&!TextUtils.isEmpty(host) && !TextUtils.isEmpty(String.valueOf(port))) {
            Proxy proxy = null;
            if(!Main.socks5Proxy) {
